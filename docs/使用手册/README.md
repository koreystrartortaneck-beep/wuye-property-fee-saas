...
底部留白裁切：按「最后一行带内容（偏暗或带彩）的位置 + 26px」裁，
**顶部 88px 是微信状态栏与导航栏，不要裁掉**。
...
- `why` / `warn` 提示框**不能带 `breakable`**。它们长在 `\step` 的 minipage 里，
  tcolorbox 会把框拆成好几段而正文一段都放不下，版面上只剩几条空色条。
- 顶层提示框如果紧跟在 `\step` 后面，而那一步的手机图占满整页，框会被顶到下一页
  独占一页。把框写进 `\step` 的第三个参数（右栏）即可。
