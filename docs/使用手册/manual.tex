...
%  插图： img/ 目录，全部取自当前代码渲染；换成真机截图只需同名覆盖，无需改本文件
% ============================================================================
...
\newtcolorbox{why}{
  enhanced, breakable, colback=cream2, colframe=gold, boxrule=0pt,
  leftrule=2.4pt, arc=2pt, left=8pt, right=8pt, top=5pt, bottom=5pt,
  fontupper=\small\color{inkgray}
}
\newtcolorbox{warn}{
  enhanced, breakable, colback=danger!5, colframe=danger, boxrule=0pt,
  leftrule=2.4pt, arc=2pt, left=8pt, right=8pt, top=5pt, bottom=5pt,
  fontupper=\small\color{inkgray}
}
...
\vspace{0.3em}
{\color{subgray}\small 适用版本 1.4.0 \quad|\quad 界面指纹 \texttt{87bac18} \quad|\quad 2026 年 8 月}\par

\vfill
\begin{why}
本手册的界面图\kw{按当前代码渲染}而成（含微信状态栏与导航栏），与你手机上看到的一致；
但它不是手机截屏，个别字体与图标可能略有差别。若某一步的界面与图不符，
说明小程序版本比手册新 —— 请以\kw{手机上的实际提示文字}为准，那些文字是随功能一起写的。
\end{why}
\end{titlepage}
...
\vspace{0.4em}
{\small\color{subgray}页面最下面那行「版本 xxxxxxx」是代码版本号，报问题时告诉物业这串字符，
他们能立刻判断你用的是哪一版。}
}
...
\begin{warn}
进不去、显示「没有管理权限」？先确认三件事：这个微信授权的是不是登记过的号码；
这个员工账号是不是\kw{在职}状态；是不是\kw{还没在电脑后台改过初始密码}（新账号必须先改一次密码）。
\end{warn}
}
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.15em]
  ...
  \item 半透明格 = 已停用的房屋（不出账、不进欠费）
\end{itemize}

顶上四个标签是今天的四件事：\kw{楼盘图 / 欠费 / 报修 / 待发布}，
数字角标告诉你哪一块有事。最上面的搜索框用来\kw{接电话查户}：房号、业主姓名、手机号都能搜。
}
...
楼盘图底部\btn{发账单}进来。三步：选\kw{标准 + 月份 + 范围（全部 / 按楼栋）}
→ \kw{预览}→ 生成草稿 → 发布。
...
首页\kw{「待发布」}标签，或点首页待办条进来。这里是所有\kw{草稿批次}
（业主还看不到的账单）。
...
楼盘图下面虚线的\kw{＋ 新增房屋}。一屏填完：房号、显示名、类型（住宅/门市/车位）、
建筑面积、放户日期、住户手机、收费标准。
...
楼盘图底部\kw{「员工与权限」}（只有\kw{物业管理员}看得到这个入口）。

\vspace{0.3em}
\begin{tabular}{@{}>{\bfseries\color{plum}}l@{\hspace{0.9em}}p{0.62\linewidth}@{}}
收费员 & 收钱、催缴、出账、报修 —— \textbf{退款与整批作废做不了}\\[0.25em]
物业管理员 & 什么都能做，含退款、整批作废、彻底删房、管员工\\
\end{tabular}
\vspace{0.4em}
...
\begin{enumerate}[leftmargin=1.4em, itemsep=0.25em]
  \item \kw{初始密码只显示一次}（系统不留明文）。忘了只能重置，重置会生成新的一次性密码。
  \item 填了手机号\kw{也不能立刻手机登录}：新账号必须\kw{先在电脑后台用初始密码登录一次并改密码}，
        之后才能用手机授权手机号免密进入。名单上会写「待首次改密」提示你。
  \item \kw{停用是即时的} —— 他手里的登录状态当场失效，不用等过期。改角色同理：
        刚被降为收费员的人，退款按钮当场就没了。
\end{enumerate}
\end{warn}
...
\vspace{0.8em}
{\small\color{subgray}
遇到界面上的文字和本手册不一致时，\textbf{以手机上的文字为准} —— 那些提示是随功能一起写的，
比手册更新得快。报问题时请一并告诉「我的」页底部那串\texttt{版本}指纹，
以及你当时点的是哪个按钮。}
