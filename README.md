...
物业人员小程序管理端改造位于 `feat/property-staff-workspace`，异机运行及 AI 接手请先读 [异机运行与 AI 接手手册](docs/异机运行与AI接手.md)，实现范围见 [物业管理端改造](docs/物业管理端改造-20260907.md)。本轮尚未部署或完成真机验收。仓库小程序默认连接生产，交互测试请先切到隔离环境；下文部分进度和演示说明为历史记录。
...
- [x] 7. 三期便捷服务：物业工作照片墙（公示监督）/ 生活服务预约（保洁清洗上门）/ 优惠券中心（物业自发券，领取核销）
- [ ] 8. 微信支付（服务商模式）与订阅消息 —— 等企业主体/商户资料
- [ ] 9. 正式版发布 —— 等备案域名 + 443 入口（云中转）
